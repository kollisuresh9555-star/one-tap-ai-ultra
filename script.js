...
async function generateVideo(promptText) {
  ...
  try {
    ...
    if (!requestId) {
      throw new Error("Video request_id was not returned");
    }
    ...
    while (!completed) {
      ...
      if (status === "COMPLETED") {
        ...
        if (!videoUrl) {
          throw new Error("Video completed but URL was not returned");
        }
      ...
      if (
        ...
        throw new Error(
          statusData.error ||
          `Video generation ${status.toLowerCase()}`
        );
      }
      ...
      if (resultBox) {
        resultBox.innerHTML = `
          <div class="video-status">
            🎬 Generating video...<br>
            <small>Status: ${status || "IN_PROGRESS"}</small>
          </div>
        `;
      }
    }
